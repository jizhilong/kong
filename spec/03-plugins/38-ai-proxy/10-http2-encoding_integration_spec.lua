    ...
    describe("http2 returns response to client", function()
      it("200 from LLM", function()
        ...
        local response_body, response = client({
          path = "/openai/llm/v1/chat",
          body = cjson.encode(format_stencils.llm_v1_chat.good.user_request),
        ...
        assert.equal(200, tonumber(response.status))

        local actual_response, err = cjson.decode(response_body)
        assert.is_falsy(err)
      ...
      it("200 from LLM but with faulty response format", function()
        ...
        -- validate that the request succeeded, response status 200
        assert.equal(500, tonumber(response.status))
      ...
      it("401 from LLM", function()
        ...
        assert.equal(401, tonumber(response.status))
        local actual_response, err = cjson.decode(response_body)
        assert.is_falsy(err)
